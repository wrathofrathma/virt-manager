...
class _XPathSegment(object):
    ...
    will be split into the following segments:
        ...
        #4: nodename=baz, condition_prop=somepro, condition_val=someval,
                fullsegment=baz[@somepro='someval']
...
class _LxmlAPI(_XMLBase):
    def __init__(self, xml):
        ...
        self._doc = etree.fromstring(xml, parser)
        self._ctx = self._doc
        for key, val in self.NAMESPACES.items():
            self._ctx.nsmap[key] = val
    ...
    def copy_api(self):
        return _LxmlAPI(etree.tostring(self._doc).decode())

    def _find(self, fullxpath):
        xpath = _XPath(fullxpath).xpath
        try:
            node = self._ctx.xpath(xpath, namespaces = self.NAMESPACES)
        ...
        resolved_node = node[0] if node else None
        return resolved_node

    def count(self, xpath):
        return len(self._ctx.xpath(xpath, namespaces=self.NAMESPACES))
    ...
    def _node_set_property(self, node, propname, setval):
        if setval is None:
            ...
            node.set(propname, escape(setval))

    def _node_new(self, xpathseg, parentnode):
        newnode = etree.Element(xpathseg.nodename)
        if not xpathseg.nsname:
            return newnode

        ns = parentnode.nsmap.get(xpathseg.nsname)
        if not ns:
            ns = self.NAMESPACES.get(xpathseg.nsname)
        if ns:
            newnode = etree.Element(f"{{{ns}}}{xpathseg.nodename}", nsmap={xpathseg.nsname: ns})
    ...
    def _node_has_content(self, node):
        return len(node) > 0 or node.attrib
